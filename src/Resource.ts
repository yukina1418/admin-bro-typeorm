...
export class Resource extends BaseResource {
  ...
  public async populate(baseRecords, property: Property) {
    const fks: Array<any> = baseRecords.map(
      (baseRecord) => baseRecord.params[property.name()]
    );

    const instances = await this.model.findByIds(fks);
    const instancesRecord: Record<string, BaseEntity> = {};
    for (const instance of instances) {
      if (instance.hasId()) instancesRecord[(instance as any).id] = instance;
    }
  ...
  public async search(
    ...
    if (meta) {
      ...
      if (prop != null) kt = { key, type: prop.type() };
    } else {
      const nameProp = this.property("name");
      const idProp = this.property("id");
      if (nameProp) kt = { key: "name", type: nameProp.type() };
      else if (idProp) kt = { key: "id", type: idProp.type() };
    }
  ...
  public async create(params: any): Promise<ParamsType> {
    const instance = await this.model.create(
      this.prepareParamsBeforeSave(params)
    );
  ...
  public async update(pk, params: any = {}): Promise<ParamsType> {
    ...
    if (instance) {
      for (const p in params) instance[p] = params[p];
